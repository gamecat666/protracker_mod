...
int8_t loadConfig(void)
{
    ...
    if (iniConfigFound)
    {
        ...
        while (configBuffer != NULL)
        {
			...
			else if (strncmp(configBuffer, "PAL_BACKGRD=", 12) == 0)
			{
				strncpy(palString, &configBuffer[12], 6);
				sscanf(palString, "%x", &palVal);
				palette[0] = palVal;
			}
			else if (strncmp(configBuffer, "PAL_BORDER=", 11) == 0)
			{
				strncpy(palString, &configBuffer[11], 6);
				sscanf(palString, "%x", &palVal);
				palette[1] = palVal;
			}
			else if (strncmp(configBuffer, "PAL_GENBKG=", 11) == 0)
			{
				strncpy(palString, &configBuffer[11], 6);
				sscanf(palString, "%x", &palVal);
				palette[2] = palVal;
			}
			else if (strncmp(configBuffer, "PAL_GENBKG2=", 12) == 0)
			{
				strncpy(palString, &configBuffer[12], 6);
				sscanf(palString, "%x", &palVal);
				palette[3] = palVal;
			}
			else if (strncmp(configBuffer, "PAL_QADSCP=", 11) == 0)
			{
				strncpy(palString, &configBuffer[11], 6);
				sscanf(palString, "%x", &palVal);
				palette[4] = palVal;
			}
			else if (strncmp(configBuffer, "PAL_PATCURSOR=", 14) == 0)
			{
				strncpy(palString, &configBuffer[14], 6);
				sscanf(palString, "%x", &palVal);
				palette[5] = palVal;
			}
			else if (strncmp(configBuffer, "PAL_GENTXT=", 11) == 0)
			{
				strncpy(palString, &configBuffer[11], 6);
				sscanf(palString, "%x", &palVal);
				palette[6] = palVal;
			}
			else if (strncmp(configBuffer, "PAL_PATTXT=", 11) == 0)
			{
				strncpy(palString, &configBuffer[11], 6);
				sscanf(palString, "%x", &palVal);
				palette[7] = palVal;
			}
			else if (strncmp(configBuffer, "PAL_PATINST=", 12) == 0)
			{
				strncpy(palString, &configBuffer[12], 6);
				sscanf(palString, "%x", &palVal);
				palette[8] = palVal;
			}
			else if (strncmp(configBuffer, "PAL_PATCMD=", 11) == 0)
			{
				strncpy(palString, &configBuffer[11], 6);
				sscanf(palString, "%x", &palVal);
				palette[9] = palVal;
			}
			else if (strncmp(configBuffer, "PAL_PATCMDVAL=", 14) == 0)
			{
				strncpy(palString, &configBuffer[14], 6);
				sscanf(palString, "%x", &palVal);
				palette[10] = palVal;
    ...
    if (ptConfigFound)
    {
        ...
        for (i = 0; i < 8; ++i)
        {
            ...
            //palette[i] = (r << 16) | (g << 8) | b;  //temporarily nobbled this so it uses ini instead- rog
        }
