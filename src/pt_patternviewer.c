        ...
        if (rowDispCheck < MOD_ROWS)
        {
            ...
            if (i == 7) // are we on the play row (middle)?
            {
                ...
                for (j = 0; j < AMIGA_VOICES; ++j)
                {
                    ...
                    {
                        if (note.sample & 0xF0)
                            printOneHexBigBg(frameBuffer, putXOffset + 30, putYOffset, note.sample >> 4, palette[PAL_PATINST], palette[PAL_GENBKG]);
                        else
                    ...
                    printOneHexBigBg(frameBuffer, putXOffset + 38, putYOffset, note.sample & 0x0F, palette[PAL_PATINST], palette[PAL_GENBKG]);
                    printOneHexBigBg(frameBuffer, putXOffset + 46, putYOffset, note.command, palette[PAL_PATCMD], palette[PAL_GENBKG]);
                    printTwoHexBigBg(frameBuffer, putXOffset + 54, putYOffset, note.param,  palette[PAL_PATCMDVAL], palette[PAL_GENBKG]);
                }
            }
            else
            {
                ...
                for (j = 0; j < AMIGA_VOICES; ++j)
                {
                    ...
                    {
                        if (note.sample & 0xF0)
                            printOneHexBg(frameBuffer, putXOffset + 30, putYOffset, note.sample >> 4, palette[PAL_PATINST], palette[PAL_BACKGRD]);
                        else
                    ...
                    printOneHexBg(frameBuffer, putXOffset + 38, putYOffset, note.sample & 0x0F, palette[PAL_PATINST], palette[PAL_BACKGRD]);
                    printOneHexBg(frameBuffer, putXOffset + 46, putYOffset, note.command, palette[PAL_PATCMD], palette[PAL_BACKGRD]);
                    printTwoHexBg(frameBuffer, putXOffset + 54, putYOffset, note.param, palette[PAL_PATCMDVAL], palette[PAL_BACKGRD]);
                }
            }
        }
    }
        ...
        if (rowDispCheck < MOD_ROWS)
        {
            ...
            if (i == 7) // are we on the play row (middle)?
            {
                ...
                for (j = 0; j < AMIGA_VOICES; ++j)
                {
                    ...
                    {
                        charOutBigBg(frameBuffer, putXOffset + 30, putYOffset, 128, palette[PAL_PATINST], palette[PAL_GENBKG]);
                        charOutBigBg(frameBuffer, putXOffset + 38, putYOffset, 128, palette[PAL_PATINST], palette[PAL_GENBKG]);
                    }

                    if ((note.command | note.param) == 0)
                    {
                        charOutBigBg(frameBuffer, putXOffset + 46, putYOffset, 128, palette[PAL_PATCMD], palette[PAL_GENBKG]);
                        charOutBigBg(frameBuffer, putXOffset + 54, putYOffset, 128, palette[PAL_PATCMDVAL], palette[PAL_GENBKG]);
                        charOutBigBg(frameBuffer, putXOffset + 62, putYOffset, 128, palette[PAL_PATCMDVAL], palette[PAL_GENBKG]);
                    }
                    else
                    {
                        printOneHexBigBg(frameBuffer, putXOffset + 46, putYOffset, note.command, palette[PAL_PATCMD], palette[PAL_GENBKG]);
            ...
            else
            {
                ...
                for (j = 0; j < AMIGA_VOICES; ++j)
                {
                    ...
                    {
                        charOutBg(frameBuffer, putXOffset + 30, putYOffset, 128, palette[PAL_PATINST], palette[PAL_BACKGRD]);
                        charOutBg(frameBuffer, putXOffset + 38, putYOffset, 128, palette[PAL_PATINST], palette[PAL_BACKGRD]);
                    }

                    if ((note.command | note.param) == 0)
                    {
                        charOutBg(frameBuffer, putXOffset + 46, putYOffset, 128, palette[PAL_PATCMD], palette[PAL_BACKGRD]);
                        charOutBg(frameBuffer, putXOffset + 54, putYOffset, 128, palette[PAL_PATCMDVAL], palette[PAL_BACKGRD]);
                        charOutBg(frameBuffer, putXOffset + 62, putYOffset, 128, palette[PAL_PATCMDVAL], palette[PAL_BACKGRD]);
                    }
                    else
                    {
                        printOneHexBg(frameBuffer, putXOffset + 46, putYOffset, note.command, palette[PAL_PATCMD], palette[PAL_BACKGRD]);
